...
#Takes string as input and converts into binary bits from the concerned base
...
#Gets the complimentary of a Binary String
def getComp(num, bin):
    detect = False
    st = ""
    bina = list(bin)
...
#Adds both the binary numbers as string input
def add_bin(bt1, bt2):
...
#This function is used for postprocessing to get final answer and map correctly into correct base as input
def pos(add, bass):
    add = add.replace("1",'',1)
    if add.startswith("1"):
...
#This function is used to add both the numbers as they are positive
...
"""
This part takes input into the program
Example 
23 -49 -> base10
14 65 ->base8
A3 -99 ->base16
""" 
#Takes th 2 numbers to work on as input
print("Enter the expression!")
n = list(map(str, input().split(" ")))
og = n

#Enter the base of both the numbers
print("Enter the base of both the numbers!")
base = int(input())

#Starting the Solution & Printing
print("\n" + "*" * 20 + "SOLUTION" + "*" * 20)

#Converting the first number into binary
print(f"\nFor first number {n[0]} we get,")
bt1 = conv_binary(n[0], base)
print(n[0].rjust(4, " ") + ": " + bt1 + " " * 4 + "<---8bit")

#Converting the second number into binary
print(f"\nFor second number {n[1]} we get,")
bt2 = conv_binary(n[1], base)
print(n[1].rjust(4, " ") + ": " + bt2 + " " * 4 + "<---8bit")

#If both the numbers are positive simply add the numbers
if "-" not in og[0] and "-" not in og[1]:
    only_add(bt1, bt2, base)
else:
    #Getting 2's compliment for the concerned negative numbers
    ...
    #Now adding the compliments to get the desired result
    print("\nNow for the addition part,")
    bt1 = bt1.replace(" ", "")
    bt2 = bt2.replace(" ", "")
    ...
    #Printing final answer and determining if answer is positive or negative
